# python AccountsCore.py -i ../../data/twitter-creator.json -d regular -f 1
# python AccountsCore.py -i ../../data/twitter-creator.json -d proxy -f 1

import time
from typing import *

from TwitterAccountBots.TwitterCore.Lib import simplejson
# import the selenium driver with pip:
...
class TwitterCreator(SeleniumHelper):
    ...
    def mobileCreateUser(self, row) -> None:
    ...
    def desktopCreateUser(self, row) -> None:
        self.loadPage(self.DESKTOP_URL_CREATE)
        self.waitAndWrite(self.DESKTOP_FIELD_SIGN_UP_NAME, row['name'])
    ...
    def desktopCreateUserPhone(self, row) -> None:
        self.loadPage(self.DESKTOP_URL_CREATE)
        self.waitAndWrite(self.DESKTOP_FIELD_SIGN_UP_NAME, row['name'])
        self.selectAndWrite(self.DESKTOP_FIELD_SIGN_UP_EMAIL, row['email'])
        self.submitForm(self.selectAndWrite(self.DESKTOP_FIELD_SIGN_UP_PASSWORD, row['password']))
    ...
    def getWebdriver(self, driverType) -> Any:
        if driverType == 'proxy':
            profile: Any = webdriver.FirefoxProfile()
    ...
    def start(self, callbacks, inputFile, fromRow, toRow, driverType) -> None:
        try:
            rows = simplejson.loads(open(inputFile).read())
            numElements: int = len(rows)
        except:
            numElements = 0
        if numElements > 0:
            if toRow == -1:
